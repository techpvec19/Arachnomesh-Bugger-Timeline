...
const getAllRoles = async (req, res) => {
    ...
    try{
        ...
        if(allRoles.length > 0) res.status(200).send(allRoles)
        else    res.status(200).send('No Roles')
    }
    catch(err){
        res.status(404).send(`Error: ${err}`);
    }
...
const getOneRole = async (req, res) => {
    ...
    try{
        ...
        if(oneRole.length < 0)  res.status(200).send('Error: Not a Valid Role Id')
        else    res.status(200).send(oneRole);
    }
    catch(err){
        res.status(404).send(`Error: ${err}`)
    }
